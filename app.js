...
if(wordleForm){
  ...
  $('wordleAddGuess')?.addEventListener('click',()=>{if((board?.children.length||0)<6)addWordleRow();});
  ...
  function collectWordleGuesses(){return Array.from(board?.querySelectorAll('.wordle-guess-row')||[]).map(row=>({word:clean(row.querySelector('.wordleGuess')?.value||''),states:Array.from(row.querySelectorAll('.wordleState')).map(el=>el.value)})).filter(row=>row.word.length===5);}
  ...
  $('wordleShare')?.addEventListener('click',async()=>{try{await navigator.clipboard.writeText(currentWordleUrl());$('wordleShare').textContent='Link copied';setTimeout(()=>$('wordleShare').textContent='Copy search link',1200)}catch{if(message)message.textContent='Could not copy the link. Copy the browser address instead.'}});
}
...
bind('wordleForm',e=>{e.preventDefault();if(!ready())return;const rows=Array.from(document.querySelectorAll('.wordle-guess-row')).map(row=>({word:clean(row.querySelector('.wordleGuess')?.value||''),states:Array.from(row.querySelectorAll('.wordleState')).map(el=>el.value)}));const partial=rows.find(row=>row.word.length>0&&row.word.length!==5);if(partial)return setResults([],'Each Wordle guess must contain exactly five letters.');const guesses=rows.filter(row=>row.word.length===5);if(!guesses.length)return setResults([],'Add at least one five-letter guess and mark its feedback.');setResults(wordleSearch({wordsByLength:state.wordsByLength,guesses,sort:'alpha'}),`Five-letter candidates consistent with ${guesses.length} guess${guesses.length===1?'':'es'}, including duplicate-letter feedback.`);});
