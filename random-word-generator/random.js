...
function hasRequiredCounts(word,required){
  ...
  const need=new Uint8Array(26),have=new Uint8Array(26);
  for(const ch of required)need[ch.charCodeAt(0)-97]++;
  for(const ch of word)have[ch.charCodeAt(0)-97]++;
...
function getPool(){
  ...
  const starts=clean($('randomStarts')?.value),contains=clean($('randomContains')?.value),ends=clean($('randomEnds')?.value),required=clean($('randomRequired')?.value),excluded=new Set(clean($('randomExcluded')?.value));
  const pool=[];for(let len=min;len<=max;len++){for(const word of state.wordsByLength.get(len)||[]){if(starts&&!word.startsWith(starts))continue;if(contains&&!word.includes(contains))continue;if(ends&&!word.endsWith(ends))continue;if(!hasRequiredCounts(word,required))continue;if([...word].some(ch=>excluded.has(ch)))continue;pool.push(word);}}
  return pool;
}
function generate(){
...
$('randomShare')?.addEventListener('click',async()=>{const url=currentUrl();try{await navigator.clipboard.writeText(url);$('randomShare').textContent='Link copied';setTimeout(()=>$('randomShare').textContent='Copy search link',1200);}catch{history.replaceState(null,'',url);message.textContent='Clipboard unavailable. The generated search URL is now in the browser address bar.';}});
